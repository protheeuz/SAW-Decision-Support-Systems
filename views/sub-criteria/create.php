...
<div class="sub-criteria-form">
    ...
    <?= $form->field($model, 'id_criteria')->dropDownList($criteriaList, ['prompt' => 'Select Criteria', 'onchange' => '
        $.post("' . Url::to(['sub-criteria/get-sub-criteria', 'id_criteria' => '']) . '" + $(this).val(), function(data) {
            $("select#subcriteria-id").html(data);
        });
    ']) ?>
    ...
    <?php elseif (Yii::$app->user->identity->role == 'Manager PMO' || Yii::$app->user->identity->role == 'Project Director'): ?>
        <?= $form->field($model, 'id')->dropDownList($subCriteriaList, ['prompt' => 'Select Sub-Criteria']) ?>
